...
export async function POST(req: Request) {
  try {
    ...
    const payload = {
      systemInstruction: {
        parts: [{ text: TRANSACTION_EXTRACTION_PROMPT }]
      },
      contents: [
        {
          role: 'user',
          parts: [{ text: "Now process the following input:\n" + text }]
        }
      ],
      generationConfig: {
        temperature: 0.1,
    ...
    if (!response.ok) {
      ...
      console.error("Gemini API Error in extract-transaction:", errorText);
    ...
    // Attempt parsing directly (Gemini is forced to return JSON by responseMimeType)
    let parsedData = {};
    try {
