...
export default function EditInventoryItemPage() {
  ...
  const { items, updateItem, isLoading } = useMasterDataStore();
  const { addNotification } = useNotificationStore();
  const [initLoading, setInitLoading] = useState(true);

  const itemId = params.id as string;

  const { register, handleSubmit, watch, reset, formState: { isSubmitting } } = useForm<Omit<Item, 'id' | 'currentStock'>>({
    defaultValues: {
      type: 'product',
      name: '',
      description: '',
  ...
  }, [isLoading, items, itemId, reset, router, addNotification]);
  ...
  if (initLoading) {
    return <div className="p-8 text-center text-slate-500 font-medium">Loading item...</div>;
  }
