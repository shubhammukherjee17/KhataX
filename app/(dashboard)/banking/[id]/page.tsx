...
export default function EditBankAccountPage() {
  ...
  const { accounts, updateAccount, isLoading } = useBankStore();
  const { addNotification } = useNotificationStore();
  const [initLoading, setInitLoading] = useState(true);

  const accountId = params.id as string;

  const { register, handleSubmit, reset, formState: { isSubmitting } } = useForm<Omit<BankAccount, 'id' | 'currentBalance'>>({
    defaultValues: {
      type: 'bank',
      name: '',
      accountNumber: '',
  ...
  }, [isLoading, accounts, accountId, reset, router, addNotification]);
  ...
  const onSubmit = async (data: Omit<BankAccount, 'id' | 'currentBalance'>) => {
    try {
      ...
      addNotification("Failed to update account.", 'error');
    }
  };

  if (initLoading) {
    return <div className="p-8 text-center text-slate-500 font-medium">Loading account...</div>;
  }
